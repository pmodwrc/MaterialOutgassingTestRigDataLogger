...
class KeithleyController:
    def __init__(self, master, num_channels=20, interval=1):
        ...
        self.psu_menu = tk.OptionMenu(self.top_right_frame, self.psu_var, *self.instruments)
    ...
    def connect_instrument(self):
        ...
        if selected_instrument != "None":
            try:
                ...
                messagebox.showinfo("Connected", f"Connected to {selected_instrument}\nIDN: {idn_response}")
                self.update_connect_button_color(self.connect_button)  # Update button color
    ...
    def connect_power_supply(self):
        ...
        if selected_psu != "None":
            try:
                ...
                messagebox.showinfo("Connected", f"Connected to {selected_psu}\nIDN: {idn_response}")
                self.update_connect_button_color(self.connect_psu_button)  # Update button color
            except pyvisa.VisaIOError:
                messagebox.showerror("Connection Error", "Failed to connect to the power supply.")
    ...
    def set_voltage(self):
        if hasattr(self, 'psu'):
            try:
                ...
                messagebox.showinfo("Voltage Set", f"Triggered Voltage set to +{voltage_positive}V and {voltage_negative}V")
            except ValueError:
        ...
        if self.pid_running:
            ...
            if channel != "None":
                try:
                    ...
                    if channel == 'CH3':
                        ch= 3
                    temperature = self.measurements[ch][-1] # get the last value from the selected channel
                    ...
                    print("T: ", temperature, "O: ", output, "  P: ", p, "  I: ", i, "  D: ", d, "  E: ", error, )
                    ...
                    messagebox.showerror("Measurement Error", "Failed to read temperature from the selected channel.")
    ...
    def set_voltage_from_pid(self, pid_output):
            ...
            if hasattr(self, 'psu'):
                try:
                    ...
                    messagebox.showerror("Power Supply Error", "Failed to set voltage on the power supply.")
    ...
    def create_channel_controls(self):
        ...
        for channel in range(1, self.num_channels + 1):  # Create controls for each channel
    ...
    def configure_channel(self, channel):
        ...
        if self.channel_vars[channel].get():
            ...
            self.instrument.write(f"ROUT:CLOS (@{channel})")  # Close the specific channel
            ...
            if config == "Voltage":
                ...
                self.instrument.write("CONF:RES")  # Configure for resistance measurement
    ...
    def measure_and_plot(self):
        ...
        while self.is_measuring:
            ...
            for channel in range(1, self.num_channels + 1):
                if self.channel_vars[channel].get():
                    ...
                    if self.channel_configs[channel].get() == "NTC_44006":
                        resistance = float(self.instrument.query("READ?"))  # Read resistance
            ...
            self.ax.xaxis.set_major_formatter(FuncFormatter(lambda x, _: datetime.fromtimestamp(x).strftime('%H:%M:%S')))
    
            # Redraw the canvas
            self.canvas.draw()
    
            latest_measurements = [self.measurements[channel][-1] if self.measurements[channel] else None for channel in range(1, self.num_channels + 1)]
    ...
    def save_measurements_as_csv(self):
        ...
        if file_path:
            with open(file_path, mode='w', newline='') as file:
                ...
                # Write header
                header = ["Timestamp"] + [f"CH{channel}" for channel in range(1, self.num_channels + 1)]
                writer.writerow(header)
    
                # Find the longest measurement list to determine rows
                max_len = max(len(self.times), max(len(v) for v in self.measurements.values()))
    
                # Write rows
                for i in range(max_len):
                    row = [self.times[i].strftime('%Y-%m-%d %H:%M:%S')] if i < len(self.times) else ""
                    row += [
                        self.measurements[channel][i] if i < len(self.measurements[channel]) else "" for channel in
                        range(1, self.num_channels + 1)]
                    writer.writerow(row)
...
if __name__ == "__main__":
    root = tk.Tk()
    app = KeithleyController(root, num_channels=3, interval=1)
    root.mainloop()
